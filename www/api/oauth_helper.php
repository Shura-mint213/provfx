...
// Fallbacks for GitLab OAuth
if (!defined('GITLAB_CLIENT_ID')) define('GITLAB_CLIENT_ID', 'placeholder_client_id');
if (!defined('GITLAB_CLIENT_SECRET')) define('GITLAB_CLIENT_SECRET', 'placeholder_client_secret');
if (!defined('GITLAB_REDIRECT_URI')) {
    $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https" : "http";
    $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
    $docRoot = realpath($_SERVER['DOCUMENT_ROOT'] ?? '');
    $helperDir = __DIR__;
    ...
    $webPath = rtrim($webPath, '/');
    define('GITLAB_REDIRECT_URI', $protocol . '://' . $host . $webPath . '/auth/gitlab/callback.php');
}
if (!defined('GITLAB_API_URL')) define('GITLAB_API_URL', 'https://gitlab.com/api/v4');
...
/**
 * Generic Token Aliases
 */
function encryptToken($token) {
    return encryptGithubToken($token);
}
...
/**
 * DB helper: Get active integration for a user
 */
function get_user_integration($student_id, $platform) {
    global $pdo;
    try {
        $stmt = $pdo->prepare("SELECT * FROM user_integrations WHERE student_id = ? AND platform = ? AND is_active = 1");
        $stmt->execute([$student_id, $platform]);
...
/**
 * DB helper: Save or update user integration
 */
function save_user_integration($student_id, $platform, $platform_user_id, $username, $email, $access_token, $refresh_token = null, $expires_at = null, $avatar_url = null, $profile_url = null, $additional_data = null) {
    global $pdo;
    
    $enc_access = encryptToken($access_token);
    $enc_refresh = $refresh_token ? encryptToken($refresh_token) : null;
    $add_data = $additional_data ? json_encode($additional_data, JSON_UNESCAPED_UNICODE) : null;
    
    $sql = "INSERT INTO user_integrations 
        (student_id, platform, platform_user_id, username, email, access_token, refresh_token, expires_at, avatar_url, profile_url, additional_data, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
...
/**
 * DB helper: Delete/disconnect integration
 */
function delete_user_integration($student_id, $platform) {
    global $pdo;
    $stmt = $pdo->prepare("DELETE FROM user_integrations WHERE student_id = ? AND platform = ?");
    $stmt->execute([$student_id, $platform]);
}
...
/**
 * Загрузка содержимого файла из GitHub репозитория
 */
...
/**
 * Определение стека технологий репозитория GitHub
 */
...
/**
 * Запрос к GitLab API через cURL
 */
function makeGitlabApiRequest($token, $url, $method = 'GET', $postData = null) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
/**
 * Загрузка содержимого файла из GitLab репозитория
 */
function fetchGitlabFileContent($token, $projectId, $path) {
    $encodedPath = urlencode($path);
    // Пробуем main, затем master
    foreach (['main', 'master'] as $ref) {
        $url = GITLAB_API_URL . "/projects/{$projectId}/repository/files/{$encodedPath}?ref={$ref}";
...
/**
 * Определение стека технологий репозитория GitLab
 */
function detectGitlabTechStack($token, $projectId, $primaryLanguage) {
    $stack = [];
    if (!empty($primaryLanguage)) {
        $stack[] = $primaryLanguage;
    }
    ...
    $files = [];
    foreach (['main', 'master'] as $ref) {
        $url = GITLAB_API_URL . "/projects/{$projectId}/repository/tree?ref={$ref}&per_page=100";
        $res = makeGitlabApiRequest($token, $url);
        if ($res['code'] === 200) {
            $files = json_decode($res['body'], true);
    ...
    if (is_array($files)) {
        ...
        if (in_array('package.json', $fileNames)) {
            ...
            $pkg = fetchGitlabFileContent($token, $projectId, 'package.json');
            if ($pkg) {
                $pkgData = json_decode($pkg, true);
                $deps = array_merge($pkgData['dependencies'] ?? [], $pkgData['devDependencies'] ?? []);
                if (isset($deps['react'])) $stack[] = 'React';
        ...
        if (in_array('requirements.txt', $fileNames) || in_array('pyproject.toml', $fileNames) || in_array('Pipfile', $fileNames)) {
            ...
            $req = fetchGitlabFileContent($token, $projectId, 'requirements.txt');
            if ($req) {
                if (stripos($req, 'django') !== false) $stack[] = 'Django';
                if (stripos($req, 'flask') !== false) $stack[] = 'Flask';
                if (stripos($req, 'fastapi') !== false) $stack[] = 'FastAPI';
        ...
        if (in_array('composer.json', $fileNames)) {
            ...
            $comp = fetchGitlabFileContent($token, $projectId, 'composer.json');
            if ($comp) {
                $compData = json_decode($comp, true);
                $deps = array_merge($compData['require'] ?? [], $compData['require-dev'] ?? []);
                if (isset($deps['laravel/framework'])) $stack[] = 'Laravel';
        ...
        if (in_array('pom.xml', $fileNames) || in_array('build.gradle', $fileNames)) {
            ...
            if (in_array('pom.xml', $fileNames)) {
                $pom = fetchGitlabFileContent($token, $projectId, 'pom.xml');
                if ($pom && stripos($pom, 'spring-boot') !== false) {
                    $stack[] = 'Spring Boot';
                }
            }
